...
int GlShellApp::LoadRGBATextureFromPNG(const char* name, unsigned int* tid, int& pngWidth, int& pngHeight)
{
#if 0
	unsigned char* pPNGData = NULL;
	pPNGData = LoadPNGImage(name,pngWidth,pngHeight);
	if(NULL == pPNGData)
	{
		printf("Open RGBA PNG File Failed!\n");
		return -1;
	}
//	printf("[%s],width=%d, height=%d\n",name,pngWidth,pngHeight);
	glGenTextures(1, tid);
	glBindTexture(GL_TEXTURE_2D,*tid);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, pngWidth, pngHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, pPNGData);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
