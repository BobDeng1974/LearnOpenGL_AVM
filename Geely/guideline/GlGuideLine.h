...
#define DIST_PER_POINT_CLOSE	0.1
...
#define DIST_LINE_WIDTH         0.09
#define SIDE_LINE_WIDTH         0.09
...
public:
	...
	void UpdateLineData(int groupNum,float* sepDis,float* LSegExtendLength,color3* groupColor,float markStartDis,
		float markDeltaDis,float markThickDis,int markNum,float trackWidth,int lineWidth,LCamera* pCam,trajectory traj,
		GuideLineType type=eStaticGuideLine,bool bUnDistort=false);
...
//	float m_fRenderLineWidth;
	...
	int m_iMarkDeltaNum;
	float m_fTrackWidth_dist;  //define the width of the distance line 
	float m_fTrackWidth_side;  //define the gap of the distance line 
};
