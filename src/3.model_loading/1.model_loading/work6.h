...
const unsigned int triangleNum = 240;
...
void AppendVertexArray(std::vector<float>& arrayVertex, float beginAngle, float endAngle, float rMin, float rMax, int rectangleNumber, float _angle)
{
    ...
    for(int i = 0; i < rectangleNumber; i++)
    {
        ...
        // bottom right
        arrayVertex.push_back(-134.0f + (float) (rMax*sin(angle*PI/180.0f)));
        arrayVertex.push_back(0.1f);
        arrayVertex.push_back(265.0f / (sin(glm::radians(float(_angle))) / cos(glm::radians(float(_angle)))) + (float) (rMax*cos(angle*PI/180.0f)));
        arrayVertex.push_back(_angle);

        // bottom left
        arrayVertex.push_back(-134.0f +(float) (rMin*sin(angle*PI/180.0f)));
        arrayVertex.push_back(0.1f);
        arrayVertex.push_back(265.0f / (sin(glm::radians(float(_angle))) / cos(glm::radians(float(_angle)))) + (float) (rMin*cos(angle*PI/180.0f)));
        arrayVertex.push_back(_angle);
...
int main()
{
    ...
    for(int angle = -45; angle <=45; angle++)
    {
        ...
        //float _angle = asin(265.0f/radius1) * 180.0 / PI;
        ...
        minR = radiu2-10.0f;
        maxR = radiu2+15.0f;
        //_angle = asin(265.0f/radiu2) * 180.0 / PI;
        AppendVertexArray(vertexArray, 0.0f, 360.0f, minR, maxR, triangleNum, (float)angle);
