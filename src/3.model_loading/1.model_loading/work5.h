...
const unsigned int triangleNum = 120;
...
//glm::vec3 at = glm::vec3(-1556.0f, 214.0f, -657.0f);
...
void AppendVertexArray(std::vector<float>& arrayVertex, float beginAngle, float endAngle, float rMin, float rMax, int rectangleNumber, float _angle)
{
...
int main()
{
    ...
    for(int angle = -45; angle <=45; angle++)
    {
        ...
        float minR = radius1-15.0f;
        float maxR = radius1+10.0f;
        //float angle = asin(265.0f/radius1) * 180.0 / PI;
        AppendVertexArray(vertexArray, 0.0f, 360.0f, minR, maxR, triangleNum, (float)angle);

        //front right
        float radiu2 = pow(pow((265.0f / (sin(glm::radians(float(angle))) / cos(glm::radians(float(angle)))) - 89), 2) + pow(265, 2), 0.5);
        minR = radiu2-5.0f;
        maxR = radiu2+15.0f;
        //angle = asin(265.0f/radiu2) * 180.0 / PI;
        AppendVertexArray(vertexArray, 0.0f, 360.0f, minR, maxR, triangleNum, (float)angle);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertexArray.size(), vertexArray.data(), GL_STREAM_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model3 = glm::translate(model3, glm::vec3(-134.0f, 0.0f, 265.0f / (sin(glm::radians(wheelAngle)) / cos(glm::radians(wheelAngle)))));
        shaderProgram.setMat4("model", model3);
        glBindVertexArray(circleVAO); // seeing as we only have a single VAO there's no need to bind it every time, but we'll do so to keep things a bit more organized
