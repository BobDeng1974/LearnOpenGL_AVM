...
const char *vertexShaderSource = "#version 330 core\n"
    ...
    "       ourColor = aColor;\n"
     "   }else{\n"
     "       ourColor = vec4(0.234f, 0.234f, 0.406f, 0.2f);\n"
     "   }\n"
    "}\0";
    ...
    for(int i = 0; i < radarNumber; i++)
    {
        for(int j = 0; j < degreeNumber; j++)
        {
            for(int k = 0; k < rectanglePeerRadarWithoutDegree; k++)
            {
                ...
                // bottom right
                printf("degreeNum+i*degreeNumber+j = %d\n", degreeNum+i*degreeNumber+j);
...
//--------------------------------------generate vertex array--------------------------------------------------------
    ...
    // draw area : left
    // rectanglePeerRadarWithoutDegree = 4;
    // radarNum = 1;
    // degreeNumber = 3;
    // AppendVertexArrayWithSector(vertexArray, indexArray, 45.0f, 135.0f, 0.4f, 0.6f, rectanglePeerRadarWithoutDegree, radarNum, degreeNumber);
    // totalNumber += rectanglePeerRadarWithoutDegree*radarNum*degreeNumber;
